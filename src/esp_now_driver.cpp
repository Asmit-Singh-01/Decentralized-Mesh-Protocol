...
#else
static void esp_now_on_data_recv(const uint8_t *mac_addr, const uint8_t *incomingData, int len) {
    EspNowDriver::handle_raw_receive(mac_addr, incomingData, len, 0);
}
#endif
...
void EspNowDriver::handle_raw_receive(const uint8_t* mac_addr, const uint8_t* incoming_data, int len, int8_t rssi) {
    if (app_rx_callback && mac_addr && incoming_data && len > 0) {
...
bool EspNowDriver::send_bytes(const uint8_t* target_mac, const uint8_t* data, size_t len) {
    ...
    const uint8_t* src = target_mac ? target_mac : dummy_mac;
    handle_raw_receive(src, data, static_cast<int>(len), -45);
    return true;
}

void EspNowDriver::set_rx_callback(void (*callback)(const uint8_t*, const uint8_t*, size_t, int8_t)) { 
    app_rx_callback = callback; 
}
