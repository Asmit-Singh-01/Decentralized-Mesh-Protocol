...
int main() {
    ...
    // 2. ESP-NOW Initialization and OTA Callback Registration
    EspNowDriver radio;

    if (!radio.init()) {
        std::cerr << "[ERROR] ESP-NOW initialization failed!" << std::endl;
        return 1;
    }

    radio.set_rx_callback(ota_updater_rx_callback);

    std::cout << "[OTA] ESP-NOW OTA updater callback registered." << std::endl;
