...
int main() {
    ...
    if (!sec.set_key(secret_key, 16)) {
        std::cerr << "[ERROR] Key setup failed!"
                  << std::endl;
    ...
    // 5. Telemetry Compression Test
    std::cout
        << "\n[COMPRESSION TEST] Testing telemetry compression..."
        << std::endl;

    MeshPacket telemetry_packet{};

    telemetry_packet.header.magic =
        PROTOCOL_MAGIC_BYTE;

    telemetry_packet.header.type =
        static_cast<uint8_t>(
            PacketType::TELEMETRY_SWARM
        );

    telemetry_packet.header.flags = 0;
    telemetry_packet.header.sender_id = 0x1001;
    telemetry_packet.header.receiver_id = 0x1002;
    telemetry_packet.header.sequence_num = 1;
    telemetry_packet.header.ttl = 5;
    telemetry_packet.header.payload_len = 64;

    /*
     * Repeated telemetry values are intentionally used here
     * because they provide a strong compression case.
     */
    int16_t telemetry_samples[32] = {
        100, 100, 100, 100,
        100, 100, 100, 100,
        100, 100, 100, 100,
        100, 100, 100, 100,
    ...
    std::memcpy(
        telemetry_packet.payload,
        telemetry_samples,
        sizeof(telemetry_samples)
    );

    uint8_t telemetry_buffer[128] = {0};
    ...
    if (serialize_packet(
        ...
        std::cout
            << "[COMPRESSION TEST] Serialized packet size: "
            << telemetry_len
            << " bytes"
            << std::endl;

        MeshPacket decoded_packet{};
        ...
        if (deserialize_packet(
            ...
            std::cout
                << "[COMPRESSION TEST] Compression round-trip successful!"
                << std::endl;

            std::cout
                << "[COMPRESSION TEST] Decoded payload: "
                << static_cast<int>(
                       decoded_packet.header.payload_len
                   )
                << " bytes"
                << std::endl;
            ...
            std::cout
                ...
                << "Failed to deserialize compressed packet."
                << std::endl;
        ...
        std::cout
            << "[COMPRESSION TEST] ERROR: "
            << "Failed to serialize telemetry packet."
            << std::endl;
